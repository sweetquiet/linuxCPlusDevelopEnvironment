...
#pragma mark 这里服务器是PF_INET
        ...
        while (1) {
            ...
            showCerts(ssl);
            sslReadFirstPackData(buf,ssl);
            sslTransmitData(buf, ssl);
            /*关闭SSL连接*/
            SSL_shutdown(ssl);
            /*释放SSL*/
            SSL_free(ssl);
...
#pragma mark 读写1
    ...
    while (1) {
        ...
        len = SSL_read(ssl, buf, MAXBUF);
        if (len==0) {
            printf("len=0 Receive Complete: 0\n");
            break;
        }else if(len<0)
        {
        ...
        {
            printf("Receive Complete2:%s\n\n",buf);
        }
        if (SSL_write(ssl, "close", len)<0) {
            perror("write:");
            return  -1;
        }
    }
