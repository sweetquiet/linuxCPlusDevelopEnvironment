...
__owur SSL_CTX* configSSLCTX(void);

int sslTransmitData(char* buf,SSL* ssl);

int sslReadFirstPackData(char*buf,SSL* ssl);
...
#pragma mark 这里客户端是AF_INET
    ...
    if (inet_aton(SERVER_IP, (struct in_addr*)&dest.sin_addr)==0) {
        perror(argv[1]);
        return -1;
    }
  ...
  //  scanf("%s",fileName);
    ...
    char* fileName = "/Users/admindyn/Desktop/dailyspace/tempmac/SOCKETLSClient/SOCKETLSClient/testcontent.txt";
    ...
    if (len<0) {
        printf("\n%s message sebd failure!Error code is %d ,Error Messages are %s \n",buffer,errno,strerror(errno));
    }
        /*
         循环发送消息内容给服务器
         */
    ...
    /*
     由于很快客户端发完消息 客户端就结束socket
     但是服务端 还没处理完 导致服务端出现Broken Pipe 报错
     */
    printf("read!\n");
    bzero(buffer, MAXBUF+1);
...
__owur SSL_CTX* configSSLCTX(void)
{
     ...
     */
    __owur const SSL_METHOD *   meth = SSLv23_client_method();
...
SSL_CTX_set_verify(ctx,SSL_VERIFY_PEER|SSL_VERIFY_FAIL_IF_NO_PEER_CERT,NULL/*verify_callback*/);
   ...
   // SSL_CTX_set_verify(ctx,SSL_VERIFY_PEER,NULL/*verify_callback*/);
     ...
     */
    int slstate = -1;
    if ((slstate=SSL_CTX_use_certificate_file(ctx, temp= CLIENT_CERT_FILE, SSL_FILETYPE_PEM))<0) {
        ERR_print_errors(stdout);
          return NULL;
    }
     ...
     */
    if ((slstate=SSL_CTX_use_PrivateKey_file(ctx, temp= CLIENT_PKEY_FILE,SSL_FILETYPE_PEM))<0) {
...
#pragma mark 这里服务器是PF_INET
    ...
    while (1) {
        ...
        if ((fd=open(strcat(file_Name_N, buf), O_CREAT|O_TRUNC|O_RDWR,0666))<0) {
            perror("open:");
            return -1;
        }
#pragma mark 读写1
