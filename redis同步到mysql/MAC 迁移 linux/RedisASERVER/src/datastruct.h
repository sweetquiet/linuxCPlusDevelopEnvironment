...
#if AS
#define AS_REDIS_IP "127.0.0.1"
#define AS_DEVICE_DB "AS_MOTE_DB"
#else
#define AS_REDIS_IP "127.0.0.1"
#define AS_DEVICE_DB "NS_MOTE_DB"
#endif
...
typedef struct st_ns_mote_info {
    ...
    uint32_t rx2_freq;//////
    uint8_t nwkkey[16];//72//appkey///////
    
    //--nwkskey
    uint8_t js_enc_key[16];//////
    uint8_t js_int_key[16];///////
    uint8_t fnwks_key[16];//120/////
    uint8_t snwks_key[16];//////
    //---
    uint8_t apps_key[16];/////
    //000
    uint8_t nwksenc_key[16];
    uint8_t multi_key[16];
    //000
    uint8_t multiaddr[4];
    uint32_t beacon_rx_slot;
    uint32_t beacon_dr;
    uint32_t beacon_freq;
    uint32_t classb_multi_slot;
    uint32_t classb_multi_dr;
    uint32_t classb_multi_freq;
    //00000
    char join_time[32];//time
    char lorawan_version[8];//1.0.2
    //
    char reserve[120];
}NS_MOTE_INFO;
/* NS_MOTE_DB*/
